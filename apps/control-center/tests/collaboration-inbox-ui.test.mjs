...
test("消息收发局拥有真实团队 scoped API 与工作面挂载点", async () => {
  ...
  assert.match(api, /teamAttention: \(teamId\)/);
  assert.match(api, /teamInboxAction: \(teamId, action\)/);
  assert.match(flow, /request\(API\.teamAttention\(team\.id\), \{ signal: controller\.signal \}\)/);
  assert.match(flow, /data-inbox-action/);
  assert.match(flow, /data-inbox-answer-form/);
  assert.match(flow, /form\.elements\.namedItem\("answer"\)/);
  assert.doesNotMatch(flow, /window\.prompt/);
  assert.match(flow, /data-revision=/);
  assert.match(flow, /expectedRevision:/);
  assert.match(server, /teamAttentionMatch/);
  assert.match(server, /collectTeamAttention/);
  assert.match(server, /teamInboxActionMatch/);
  assert.match(server, /inboxLifecycle\.apply/);
  assert.match(server, /inbox answer text is required/);
  const actionBlock = server.slice(server.indexOf("teamInboxActionMatch"));
  assert.ok(actionBlock.indexOf("bus.append") < actionBlock.indexOf("inboxLifecycle.apply"), "answer/ack must hit bus before lifecycle");
  ...
  assert.match(html, /role="radiogroup" aria-label="任务类型"/);
  assert.match(html, /role="radio" aria-checked="true"[^>]+data-router-kind="auto"/);
  assert.equal((html.match(/role="radio" aria-checked="false" class="team-router-kind" data-router-kind=/g) || []).length, 6);
