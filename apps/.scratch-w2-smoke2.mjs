...
import { ProviderStore } from "./control-center/src/providers.mjs";

const root = mkdtempSync(join(tmpdir(), "prov-w2-"));
const store = await new ProviderStore({ dataRoot: join(root, "data"), runtimeHome: join(root, "home") }).init();
...
await store.setCommonConfig("claude", JSON.stringify({ attribution: { commit: "", pr: "" }, env: { CLAUDE_CODE_EFFORT_LEVEL: "max", ENABLE_TOOL_SEARCH: "true" } }));
