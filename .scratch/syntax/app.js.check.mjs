...
const REMOTE_PROJECT_EMPTY_HINT = "远程项目：行尾「写」按钮打开 SSH 终端；远端 agent 会话属 v41 波";

// 远程项目节点：globe 图标 + 主机徽标（状态点如实：已连接=台账 trusted，已停用/主机缺失压暗）
function remoteProjectNodeMarkup(project, idSuffix, { pinned = false } = {}) {
  ...
  return `<div class="project-node is-remote-project${pref.hidden ? " is-hidden-project" : ""}${pinned ? " pinned-project" : ""}">
    <div class="project-row">
      ...
      <button class="row-action" type="button" data-project-newsession="${escapeHtml(project.id)}"
        title="打开该远程目录的 SSH 终端" aria-label="打开「${escapeHtml(pref.name || project.label)}」的 SSH 终端"${remote.hostMissing || !remote.enabled ? " disabled" : ""}>
        <svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="${MENU_ICONS.rename}" /></svg>
      </button>
      ${menuTriggerMarkup("project", project.id, `打开「${pref.name || project.label}」项目菜单`)}
    </div>
    <ul class="project-sessions" id="${listId}"${expanded ? "" : " hidden"}>
      ${expanded ? `<li class="project-empty">${REMOTE_PROJECT_EMPTY_HINT}</li>` : ""}
    </ul>
  </div>`;
}
...
function toggleProject(id, button) {
  ...
  if (wasExpanded) list.replaceChildren();
    ...
    const items = project ? sessionGroupsMarkup(project, visibleTreeSessions(project)) : "";
  ...
  document.addEventListener("click", (event) => {
    ...
    if (showMore) {
      ...
      if (project && list) {
        ...
        list.innerHTML = sessionGroupsMarkup(project, visibleTreeSessions(project)) || `<li class="project-empty">无历史对话</li>`;
    ...
    if (projectNewSession && !projectNewSession.disabled) {
      ...
      if (project?.remoteProject) {
        // 远程项目：「写」= 打开落在该项目远程目录的 SSH 终端（v41 才接远端 agent 会话）
        void openRemoteTerminalForProject(project);
