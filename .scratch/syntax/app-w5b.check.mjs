...
/* ===== 配置目标主机切换（v41 波四「配置图谱里直接配置远程主机」→ 波五远程三面图谱） =====
 * 默认配本机（三面图谱照旧）；切到 SSH 台账主机后，拓扑导航保留可切，本机三面板让位给远程面板，
 * 远程面板按当前 surface 出远端实况（供应商浅提取/能力清单/CLI 矩阵+真源），
...
function selectConfigHost(id) {
  ...
  if (state.configHostResult && state.configHostResult.hostId !== state.configHostId) state.configHostResult = null; // 换主机/回本机清内联结果
...
function configRemoteResult(html) {
  state.configHostResult = state.configHostId ? { hostId: state.configHostId, html } : null;
...
function renderConfigRemotePanel() {
  ...
  const resultHtml = state.configHostResult?.hostId === host.id ? state.configHostResult.html : "";
  panel.innerHTML = `
    <div class="config-surface-heading">
      <div>
        <p class="eyebrow">Remote Host · ${escapeHtml(CONFIG_SURFACE_LABELS[state.configSurface] ?? "")}</p>
      ...
      <div class="config-remote-actions">
        ...
        <button type="button" class="button secondary" data-config-host-graph="${escapeHtml(host.id)}"><svg class="icon lucide"><use href="#lucide-scan-search"></use></svg> 刷新图谱</button>
    ...
    <div class="config-remote-result" aria-live="polite">${resultHtml}</div>
    ${configRemoteSurfaceBody(host)}`;
}
...
function configRemoteSurfaceBody(host) {
  if (state.configSurface === "capabilities") return configRemoteCapsBody(host);
  if (state.configSurface === "sources") return configRemoteSourcesBody(host);
  return configRemoteProvidersBody(host);
}

/** graph 三态壳：loading / error 如实呈现，ok 才进 render(data)。 */
function configRemoteGraphBody(host, render) {
  const entry = state.configHostGraph.get(host.id);
...
/** surface ①：供应商与应用——远端 live 配置浅提取（只读，写通道=头部同步本机配置）。 */
function configRemoteProvidersBody(host) {
  return configRemoteGraphBody(host, (graph) => {
    const rows = (graph.providers ?? []).map((row) => `<tr>
      <td>${escapeHtml(remoteCliLabel(row.cli))}</td>
      <td><code>${escapeHtml(row.file)}</code></td>
      <td>${row.exists ? `<span class="status-label is-ok">live</span>` : `<span class="subtle">未配置</span>`}</td>
      <td>${escapeHtml(row.model ?? "—")}</td>
      <td>${row.baseUrl ? `<code>${escapeHtml(row.baseUrl)}</code>` : "—"}</td>
      <td>${escapeHtml(row.wireApi ?? row.provider ?? "—")}</td>
    </tr>`).join("");
    return `
      <table class="data-table config-remote-providers">
        <thead><tr><th>应用</th><th>live 配置</th><th>状态</th><th>模型</th><th>Base URL</th><th>协议</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
      <p class="subtle">远端 <code>${escapeHtml(graph.home ?? "$HOME")}</code> 下 live 配置的浅提取（只读 model / Base URL / 协议字段，key 类字段永不提取，坏文件如实留空）。写入通道 = 头部「同步本机配置」；供应商档案远端投影留待后续波次。</p>`;
  });
}
...
function configRemoteCapsBody(host) {
  return configRemoteGraphBody(host, (graph) => {
    ...
    const groups = CONFIG_REMOTE_CAP_GROUPS.map((group) => {
      const items = caps.filter((cap) => cap.kind === group.kind);
      const chips = items.length
        ? items.map((cap) => `<span class="config-cap-chip" title="${escapeHtml(remoteCliLabel(cap.cli))}">${escapeHtml(cap.name)}</span>`).join("")
        : `<span class="subtle">无（按已知目录浅扫描）</span>`;
      return `<div class="config-cap-group">
        <div class="config-cap-group-head"><svg class="icon lucide"><use href="#lucide-${group.icon}"></use></svg> ${group.label}<span class="subtle">${items.length}</span></div>
        <div class="config-cap-chips">${chips}</div>
      </div>`;
    }).join("");
    const mcp = graph.mcp ?? [];
    const mcpChips = mcp.length
      ? mcp.map((entry) => `<span class="config-cap-chip is-mcp" title="${escapeHtml(`${remoteCliLabel(entry.cli)} · ${entry.source}`)}">${escapeHtml(entry.name)}</span>`).join("")
      : `<span class="subtle">无（live 配置内未发现 mcpServers / [mcp_servers.*]）</span>`;
    return `${groups}
      <div class="config-cap-group">
        <div class="config-cap-group-head"><svg class="icon lucide"><use href="#lucide-webhook"></use></svg> MCP<span class="subtle">${mcp.length}</span></div>
        <div class="config-cap-chips">${mcpChips}</div>
      </div>
      <p class="subtle">按已知目录浅扫描（Claude .claude/agents|skills|commands · Codex .codex/prompts · Kimi .agents/skills），不深入项目级目录。</p>`;
  });
}
...
function configRemoteSourcesBody(host) {
  const envCard = `
    <div class="sshconn-detail config-remote-env">
      <div class="sshconn-detail-head"><strong><svg class="icon lucide"><use href="#lucide-activity"></use></svg> 远程环境（运行席位）</strong></div>
      ${configRemoteEnvBody(host)}
    </div>`;
  return envCard + configRemoteGraphBody(host, (graph) => {
    const openSel = state.configHostSourceOpen;
    const rows = (graph.sources ?? []).map((file) => {
      const isOpen = openSel?.hostId === host.id && openSel?.fileId === file.id;
      let detail = "";
      if (isOpen) {
        const cache = state.configHostSourceCache.get(`${host.id}:${file.id}`);
        if (!cache || cache.status === "loading") {
          ...
          detail = `<pre class="waveg-log">${escapeHtml(data.content ?? "")}</pre>`
            + (data.truncated ? `<p class="subtle">内容超过 1MB，仅显示前段（已脱敏）。</p>` : "");
        }
      }
      const meta = file.exists
        ? `${formatRemoteSize(file.size)}${file.mtime ? ` · ${escapeHtml(file.mtime.replace("T", " ").slice(0, 16))}` : ""}`
        : "不存在";
      return `<div class="config-source-row${file.exists ? "" : " is-missing"}${isOpen ? " is-open" : ""}">
        <button type="button" class="config-source-open" data-config-graph-file="${escapeHtml(file.id)}" ${file.exists ? "" : "disabled"}>
          <svg class="icon lucide"><use href="#lucide-file-text"></use></svg>
          <span class="config-source-label">${escapeHtml(file.label)}</span>
          <code>${escapeHtml(file.remote)}</code>
    ...
    return `
      <div class="config-source-list">${rows}</div>
      <p class="subtle">真源内容经 SFTP 读取：路径围栏（仅 $HOME 表内文件）+ 1MB 截断 + 秘密脱敏，不存在文件不可点开。</p>`;
  });
}
...
/** 远程图谱拉取：选中主机即加载，「刷新图谱」强制重拉；三态进 configHostGraph Map。 */
async function loadConfigHostGraph(hostId, { force = false } = {}) {
  if (!force && state.configHostGraph.has(hostId)) return;
  state.configHostGraph.set(hostId, { status: "loading" });
  if (state.configHostId === hostId) renderConfigRemotePanel();
  try {
    const result = await request(`/api/ssh/hosts/${hostId}/graph`);
    state.configHostGraph.set(hostId, { status: "ok", data: result ?? null });
  } catch (error) {
    state.configHostGraph.set(hostId, { status: "error", error: error.message });
  }
  if (state.configHostId === hostId) renderConfigRemotePanel();
}

/** 真源文件点击展开/收起；内容按 hostId:fileId 缓存，再点同一行收起。 */
async function toggleConfigGraphFile(hostId, fileId) {
  const open = state.configHostSourceOpen;
  if (open?.hostId === hostId && open?.fileId === fileId) {
    state.configHostSourceOpen = null;
    renderConfigRemotePanel();
    return;
  }
  state.configHostSourceOpen = { hostId, fileId };
  const key = `${hostId}:${fileId}`;
  if (!state.configHostSourceCache.has(key)) {
    state.configHostSourceCache.set(key, { status: "loading" });
    renderConfigRemotePanel();
    try {
      const result = await request(`/api/ssh/hosts/${hostId}/graph/source?file=${encodeURIComponent(fileId)}`);
...
function bindEvents() {
  ...
  document.addEventListener("click", (event) => {
    ...
    const configHostGraph = event.target.closest("[data-config-host-graph]");
    if (configHostGraph) void loadConfigHostGraph(configHostGraph.dataset.configHostGraph, { force: true });
    const configGraphFile = event.target.closest("[data-config-graph-file]");
    if (configGraphFile && state.configHostId) void toggleConfigGraphFile(state.configHostId, configGraphFile.dataset.configGraphFile);
