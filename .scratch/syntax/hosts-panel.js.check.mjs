      ...
      <div class="sshconn-actions">
        ...
        <div class="sshconn-menu-wrap">
          ...
          <div class="sshconn-menu" hidden>
            ...
            <button type="button" data-host-syncconfig="${host.id}">${lucideIcon("cloud-upload", "icon lucide")} 同步本机配置</button>
          ...
          <div class="sshconn-cli${cli.installed ? "" : " is-missing"}">
            ...
            ${cli.installed
              ...
              : `<span class="subtle">未安装</span>
                 <button type="button" class="button mini" data-install-cli="${host.id}:${cli.id}">安装</button>`}
          </div>`).join("")}
      </div>`;
  }
...
async function openSyncConfigDialog(root, host) {
  ...
  let plan;
  try {
    plan = await request(`/api/ssh/hosts/${host.id}/sync-config/plan`);
  } catch (error) {
  ...
  dialog.querySelector('[data-act="push"]')?.addEventListener("click", async () => {
    ...
    try {
      const result = await request(`/api/ssh/hosts/${host.id}/sync-config`, { method: "POST", body: JSON.stringify({ files: selected }) });
      dialog.close();
      actionResult(root, `
        <div class="waveg-review">
          <strong>${lucideIcon("cloud-upload", "icon lucide")} 同步完成（远端 HOME=${esc(result?.home ?? "?")}）</strong>
